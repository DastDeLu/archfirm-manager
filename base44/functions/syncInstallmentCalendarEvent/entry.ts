...
Deno.serve(withAuth(async (req) => {
  ...
  // Load through user-scoped client; platform permissions already scope to the caller.
  ...
  // Explicit ownership gate (enforced post-migration).
  ...
  const kindLabel = installment.kind === 'acconto' ? 'Acconto'
    : installment.kind === 'saldo' ? 'Saldo'
